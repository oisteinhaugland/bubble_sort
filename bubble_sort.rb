...
def bubbleSort(numbers)
	...
	while numbersToCheck != 0
		newLength = 0 #magic - if i never gets changed the array is sorted
...
testArray = ["longsetWordOftheDay","hello","hola","hi","hombre","nordmøre"]

def bubble_sort_by(array)
	n = array.size
	wordsToCheck = array.size
	while wordsToCheck != 0
		newLength = 0 
		for i in (1..wordsToCheck-1)
			if yield(array[i-1],array[i]) > 0
				array[i-1], array[i] =  array[i], array[i-1]
				newLength = i
			end
		end
		wordsToCheck = newLength
	end
	p array
end

bubble_sort_by(testArray) do |left,right|
